...
- **Production boundary:** deployed builds use Clerk without identity fallback. New users choose Student or Teacher during sign-in/sign-up; both roles are activated through a server-verified Clerk session without an approval queue. Students can later promote themselves to Teacher, while classroom membership and ownership checks still protect data. Production execution uses authenticated HTTPS runner services with bounded queues; loopback Java and C++ workers remain development-only. Practical authoring intentionally uses the current single-problem data model.
...
- **Reliable demo:** `npm run demo:fresh` fully restores the isolated loopback demo database and starts the deterministic, clearly labeled simulated execution experience without requiring Docker. When Docker and both pinned images are available, `npm run demo:real` routes Java/C++ through separate workers without falling back to simulation. **Preview as teacher/student** changes presentation only and is not authentication.
...
Configure Clerk to deliver `user.created`, `user.updated`, and `user.deleted` events to `/api/webhooks/clerk`. Enable Email, Google, and GitHub connections in Clerk. TRACE creates the selected Student or Teacher role only after verifying the signed-in Clerk session and verified primary email; URL parameters and webhook unsafe metadata are never sufficient by themselves.

For low-latency local development, use the one-command isolated PostgreSQL workflow:

```bash
npm run dev:local
```

`dev:local` starts the pinned Docker database when Docker is available. On Windows, it can instead initialize a workspace-owned cluster from an installed PostgreSQL distribution; it never alters the system PostgreSQL services. Both paths listen only on `127.0.0.1:54329`, apply migrations, regenerate Prisma Client, seed only the isolated demo database, verify fixtures, and then open Next.js. The separate `labrix_test` database is reserved for destructive verification. If fixtures are incomplete, startup stops instead of silently resetting work; `npm run db:local:prepare` is the explicit recovery command. Use `npm run db:local:down` to stop the selected local database without deleting its data. Keep shared/staging database URLs out of local development processes.
...
For a 40–50 student club rehearsal, configure the deployed origin, private
diagnostics token, and runner credentials, then run `npm run verify:club-capacity`.
Immediately before a session, record the latest backup, restore drill, and real
teacher/student smoke-test timestamps and run `npm run verify:preclass`. Both
commands fail closed when required evidence is missing; see
[documentation/12-OPERATIONS-RUNBOOK.md](documentation/12-OPERATIONS-RUNBOOK.md).
