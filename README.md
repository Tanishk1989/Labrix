...
Teacher approval email also requires `LABRIX_APP_URL`, a Resend API key, the administrator recipient and verified sender addresses, and a random `TEACHER_APPROVAL_SECRET` of at least 32 characters. Configure Clerk to deliver `user.created`, `user.updated`, and `user.deleted` events to `/api/webhooks/clerk`.
