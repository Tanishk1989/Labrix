...
Initial authentication uses verified email and password with Clerk Hobby session defaults. Students may self-register locally as `STUDENT` and require a valid join code for classroom access. No browser role choice can grant `TEACHER`: a Clerk administrator assigns teacher metadata, the signed Clerk webhook creates or converts the local identity in `PENDING_TEACHER_APPROVAL`, and a signed, expiring email capability lets the configured administrator activate that specific request. Teacher access remains denied until activation. Separate Clerk development and production instances are required. MFA and Clerk email invitations remain deferred. The fixed seeded demo resolver remains current during the transition and must be unavailable in production once authenticated resolution is implemented.
...
**Accepted 2026-08-10; amended 2026-08-12.** Each immutable submission attempt may have one current teacher review. The practical owns a configurable maximum mark and may define 2–5 criteria whose maxima exactly total that mark; overall-only grading remains supported. Rubrics cannot change after student activity. Publishing requires written feedback and explicit confirmation. Every draft or publish save creates an append-only revision snapshot. Drafts and revision history remain teacher-only; only the current published review is visible to the student who owns that attempt. Students cannot create or edit reviews. Weighted criteria, moderation workflows, and AI-generated feedback remain deferred.

### D-015 — Test visibility and assessment model

**Amended 2026-08-12.** Test cases are optional for a single-problem practical. When tests exist, Run evaluates visible tests only and Submit evaluates both visible and hidden groups. Students receive visible detail and only a hidden pass/total aggregate, while the classroom-owning teacher may inspect hidden details. A testless Run or Submit is recorded without inventing a pass verdict or automated score. When tests exist, each new immutable result snapshot stores a deterministic equal-weight suggested score out of ten rounded to one decimal; provider/compile/runtime failure scores zero. Suggested scoring never replaces teacher-awarded marks. Teacher-authored rubrics are governed by D-019; weighted automated grading remains out of scope.
