...
No student source runs in Next.js. Development defaults to disclosed deterministic simulation. Production writes durable PostgreSQL execution jobs; a separate worker claims jobs with leases and bounded retries, selects the Java or C++ authenticated HTTPS adapter, and persists immutable results. Runner hosts create disposable constrained containers per request. The web process never invokes a compiler, Docker, a shell, or child process.
