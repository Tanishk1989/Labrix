...
**Status:** complete for basic marks and written feedback. Configurable rubrics and review history were added later in Phase 20A; weighted automated grading and AI feedback remain excluded.
...
**Status:** complete with the deterministic simulated provider. Real compilation and weighted automated grading remain excluded; teacher-authored rubrics were added later in Phase 20A.
...
**Status:** complete for the existing teacher submissions route. Configurable rubrics and review history were added later in Phase 20A; notifications, weighted automated grading, AI, and production execution remain excluded.
...
## Phase 20A — configurable teacher grading and review history

- Let each practical define a maximum mark and optionally use 2–5 teacher-authored criteria whose marks total that maximum.
- Keep the current review mutable while recording every draft and publish save as an append-only revision.
- Keep review drafts and revision history teacher-only; expose only the current published marks, rubric scores, and feedback to the owning student.
- Lock the grading contract after student activity and keep deterministic test scoring separate from teacher-awarded marks.
