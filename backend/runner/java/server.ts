    ...
    try {
      ...
      if (!response.destroyed) {
        ...
        if (error?.message === "QUEUE_FULL") {
          writeJson(response, 503, { error: "The local Java runner queue is full. Try again shortly." });
        } else if (error?.message === "QUEUE_TIMEOUT") {
          writeJson(response, 504, { error: "Request timed out waiting for an available Java runner." });
        } else {
          writeJson(response, 500, { error: "The local Java runner failed unexpectedly." });
        }
      }
    } finally {
