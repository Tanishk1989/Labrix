...
import {
  JAVA_RUNNER_COMPILE_TIME_MS,
  JAVA_RUNNER_OUTPUT_BYTES,
  JAVA_RUNNER_TOTAL_OUTPUT_BYTES,
} from "../../server/execution/java-runner-protocol";

export const JAVA_RUNNER_IMAGE =
...
const CONTAINER_LIFETIME_SECONDS = 15;
...
function runDocker(
  ...
  {
    input,
    maxStdoutBytes = JAVA_RUNNER_OUTPUT_BYTES,
    maxStderrBytes = JAVA_RUNNER_OUTPUT_BYTES,
    signal,
    timeoutMs,
...
export async function cleanupActiveJavaContainers() {
  ...
  try {
    const created = await runDocker(
      [
        ...
        "--tmpfs",
        "/workspace:rw,nosuid,nodev,noexec,size=33554432,uid=65532,gid=65532,mode=0700",
        "--tmpfs",
    ...
    if (created.exitCode !== 0 || created.timedOut || created.aborted) {
      return internalError(
        request,
        commandError(created, "Docker could not create the Java sandbox."),
      );
    }
    ...
    if (started.exitCode !== 0 || started.timedOut || started.aborted) {
      return internalError(
        request,
        commandError(started, "Docker could not start the Java sandbox."),
      );
    }
    ...
    const sourceWritten = await runDocker(
      [
        ...
        "/bin/sh",
        "-c",
        "umask 077; mkdir -p /workspace/classes; cat > /workspace/Main.java",
      ],
      {
    ...
    if (
      ...
      return internalError(
        ...
        commandError(
          sourceWritten,
          "The Java source could not be staged safely.",
        ),
      );
    }
    ...
    const compilation = await runDocker(
      ...
      { signal, timeoutMs: JAVA_RUNNER_COMPILE_TIME_MS },
    );
    if (compilation.aborted) {
      return internalError(request, "The Java execution request was aborted.");
    }
    if (compilation.timedOut) {
      return {
        ...
        passedTests: 0,
        totalTests: request.tests.length,
        errorText: "Java compilation exceeded the local compiler deadline.",
        testResults: [],
      };
    }
    if (compilation.exitCode !== 0 || compilation.outputLimitExceeded) {
      return {
        ...
        errorText: commandError(
          compilation,
          "Java compilation failed without compiler output.",
        ),
        testResults: [],
      };
    }

    const testResults: JavaRunnerResponse["testResults"] = [];
    ...
    for (const test of request.tests) {
      ...
      if (remainingTimeMs <= 0) {
        return {
          ...
          totalTests: request.tests.length,
          errorText: "Java execution exceeded the 2000 ms wall-clock limit.",
          testResults,
        };
      }
      ...
      const remainingOutputBytes = Math.max(
        1,
        JAVA_RUNNER_TOTAL_OUTPUT_BYTES - returnedOutputBytes,
      );
      ...
      if (execution.outputLimitExceeded) {
        return {
          ...
          passedTests: testResults.filter((result) => result.passed).length,
          totalTests: request.tests.length,
          errorText: "Java output exceeded the configured capture limit.",
          testResults,
        };
      }
      if (execution.exitCode !== 0) {
        return {
          ...
          errorText: commandError(
            execution,
            "Java exited with a non-zero status without error output.",
          ),
          testResults,
        };
      }
    ...
    return internalError(
      ...
      error instanceof Error
        ? error.message
        : "The Java sandbox failed unexpectedly.",
    );
  } finally {
