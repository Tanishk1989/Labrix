...
export interface TeacherPracticalInput {
  ...
  deadline: Date | null;
  maximumMarks?: number;
  rubricCriteria?: Array<{ title: string; maximumMarks: number }>;
  testCases: Array<{
    input: string;
...
export async function saveTeacherPractical(input: TeacherPracticalInput) {
  return prisma.$transaction(
    async (tx) => {
      ...
      const current = input.taskId
        ? await tx.task.findFirst({
            ...
            select: {
              ...
              rubricCriteria: { orderBy: { position: "asc" }, select: { id: true, title: true, maximumMarks: true } },
            },
          })
        : null;
      ...
      const requestedRubric = input.rubricCriteria ?? [];
      const rubricChanged = current !== null && (
        current.maximumMarks !== (input.maximumMarks ?? 10) ||
        current.rubricCriteria.length !== requestedRubric.length ||
        current.rubricCriteria.some((criterion, index) => {
          const next = requestedRubric[index];
          return !next || criterion.title !== next.title.trim() || criterion.maximumMarks !== next.maximumMarks;
        })
      );
      if (current && hasStudentActivity && rubricChanged) {
        throw new PracticalAuthoringError(
          "TESTS_LOCKED",
          "The grading rubric cannot be changed because student work already exists.",
        );
      }
      ...
      if (!hasStudentActivity) {
        await tx.rubricCriterion.deleteMany({ where: { taskId: task.id } });
        if (requestedRubric.length > 0) {
          await tx.rubricCriterion.createMany({
            data: requestedRubric.map((criterion, index) => ({
              taskId: task.id,
              position: index + 1,
