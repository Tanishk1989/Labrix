...
async function loadTeacherOverview(teacherId: string): Promise<TeacherOverview> {
  ...
  const studentDetails = new Map<string, { name: string; email: string; classroomIds: Set<string>; latestActivityAt: string | null }>();
    ...
    for (const membership of classroom.memberships) {
      ...
      studentDetails.set(membership.userId, {
        ...
        classroomIds: new Set([
          ...(studentDetails.get(membership.userId)?.classroomIds ?? []),
          classroom.id,
        ]),
    ...
    if (latestPublished) {
      ...
      if (missingCount > 0) {
        attention.push({
          ...
          href: `/progress?classroom=${encodeURIComponent(classroom.id)}`,
  ...
  const students = [...studentDetails.entries()]
    .map(([id, details]) => {
      ...
      return {
        ...
        name: details.name,
        email: details.email,
        classroomIds: [...details.classroomIds],
        latestActivityAt: details.latestActivityAt,
