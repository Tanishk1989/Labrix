...
When `NODE_ENV=production`, selecting `java-http` or `cpp-http` throws a configuration error before a provider is returned. The exceptional override is deliberately explicit and exact:
