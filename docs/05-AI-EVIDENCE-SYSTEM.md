...
Labrix selects group membership before the provider call. Top verified performers have a latest score of at least 8.0/10, a published review, no high review-priority category, and no failed hidden aggregate when one is available. Needs attention covers no submission, score below 5.0/10, hidden aggregate failure, high review priority, or review not published. Names remain in the teacher-only server result and are rendered outside AI output. AI can explain the supplied criteria but cannot add/remove students or generate evidence/signals.

Phase AI-5 renders the deterministic groups directly on the owner-only practical progress page without invoking a provider. Each group is ordered reproducibly and limited to five displayed students while retaining its total count. Rows show neutral reason badges, attempt/submission state, suggested score, hidden pass/total aggregate or unavailable state, review status, and a review link. Hidden test identifiers, inputs, expected output, and actual output never enter this DTO.
