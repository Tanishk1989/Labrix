...
export async function submitStudentOnboarding(
  ...
  try {
    ...
    return { message: "Labrix could not complete onboarding. Please try again." };
  }

  if (result.ok) redirect("/classes");
  ...
  const messages = {
    ...
    EMAIL_IN_USE:
      "A Labrix account already uses this email. Ask an administrator to resolve it; Labrix will not link accounts by email.",
    CONFLICT: "Labrix could not complete onboarding safely. Please try again.",
  } as const;
