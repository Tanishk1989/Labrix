...
export default async function SubmissionReviewPage({ params }: { params: Promise<{ submissionId: string }> }) {
  ...
  return <DemoShell actor={actor}><div className="space-y-4">
    <header className="flex flex-wrap items-start justify-between gap-4"><div className="flex items-start gap-3"><Link href="/submissions" className="icon-button" aria-label="Back to submissions"><ArrowLeft size={16} /></Link><div><p className="eyebrow">Teacher review · Immutable attempt</p><h1 className="page-heading">{review.student.name}</h1><p className="page-subtitle">{review.task.title} · {review.task.classroom.name} · Attempt #{review.attemptNumber} · {review.language}</p></div></div><div className="text-right"><div className="flex justify-end gap-2"><SubmissionTimingBadge status={review.timingStatus}/><StatusBadge tone={passedAll ? "passed" : review.result.state === "compilation_error" ? "compilation-error" : "warning"}>{passedAll ? "Passed all provided tests" : review.result.state.replaceAll("_", " ")}</StatusBadge></div><p className="mt-2 text-[11px] text-[var(--text-muted)]">Submitted {new Date(review.submittedAt).toLocaleString("en-IN")}</p></div></header>
