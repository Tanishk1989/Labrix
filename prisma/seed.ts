...
async function main() {
  const teacher = await prisma.user.upsert({
    ...
    update: { name: "Dr. Sharma", email: "dr.sharma@demo.codeclass.local", platformRole: PlatformRole.TEACHER },
    create: { id: "demo-teacher", name: "Dr. Sharma", email: "dr.sharma@demo.codeclass.local", platformRole: PlatformRole.TEACHER },
  });
  ...
  for (const [id, name] of [["demo-student-1", "Aarav Mehta"], ["demo-student-2", "Diya Sharma"], ["demo-student-3", "Kabir Singh"]]) {
    const student = await prisma.user.upsert({ where: { id }, update: { name, email: `${id}@demo.codeclass.local`, platformRole: PlatformRole.STUDENT }, create: { id, name, email: `${id}@demo.codeclass.local`, platformRole: PlatformRole.STUDENT } });
